...
describe('PersonalLoan', () => {
  test('.toJSON()', () => {
    ...
    const expected = { type: 'PERSONAL', interest_rate: 4 }
  ...
  describe('.matchProfile()', () => {
    test('Income less than or equal to 3000', () => {
      ...
      const personal = new PersonalLoan()

      expect(personal.matchProfile(customer)).toBe(true)
    })
    ...
    test('Invalid income range', () => {
      ...
      expect(personal.matchProfile(customer)).toBe(false)
    })
    test('Invalid age', () => {
      const customer = {
        age: 30,
